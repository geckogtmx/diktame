...
3. **Gemma3:1b warmup on settings reload (VRAM impact)** — Every settings save triggers full provider re-init → warms up gemma3:1b into VRAM even when it's NOT the configured LLM. Root cause: `SettingsManager: loaded settings` → full re-init → `OllamaProvider.WarmUpAsync()` always targets `gemma3:1b` regardless of which model the user actually has set as their local LLM. Fix should: (a) only warm up the user's *configured* local LLM model, not a hardcoded one; (b) skip warmup on settings reload if the model hasn't changed; (c) only run warmup on first app startup. This wastes ~1.2GB VRAM by keeping gemma3:1b loaded alongside the user's actual LLM (e.g., lfm2.5). Fixing this keeps steady-state VRAM under 4GB (Whisper ~1GB + one LLM ~1.2-2.7GB + vision on-demand ~2GB for 5min).
