# dIKta.me V2 - User Documentation

Welcome to the official user documentation for dIKta.me V2! 

The application is a massive leap forward from V1, transitioning into a fully native Windows desktop application to bring unprecedented stability, speed, and deep OS integration.

Below you will find comprehensive guides on every feature, pipeline, and setting inside the application.

## 🚀 Getting Started

If you are new here, start with this critical guide.
*   [Getting Started Guide](getting-started.md)

## 🎙️ Core Features

Learn how to use the primary dictation systems and text pipelines.
*   [Dictation Modes (Batch vs Streaming)](features/dictation.md)
*   [The Refine Pipeline](features/refine.md)
*   [The Ask Pipeline](features/ask.md)
*   [Real-time Translation](features/translate.md)
*   [Voice Notes](features/note.md)
*   [Quick Chat Overlay](features/quick-chat.md)
*   [The "Oops" Hotkey](features/oops.md)

## ⚙️ Settings Exhaustive Guide

A deep dive into everything you can configure within the multi-tab Settings Window.
*   [General Settings](settings/general.md)
*   [Account & Wallets](settings/account.md)
*   [API Keys & BYOK](settings/api-keys.md)
*   [Global AI Engine](settings/ai-engine.md)
*   [Audio & Ducking](settings/audio.md)
*   [Custom Dictation Modes](settings/dictation-modes.md)
*   [Utility Modes Configuration](settings/modes.md)
*   [Global Hotkeys](settings/hotkeys.md)
*   [Text Snippets](settings/snippets.md)
*   [Ollama Local Inference](settings/ollama.md)
*   [Privacy Controls & History](settings/privacy.md)
*   [Control Panel HUD](settings/control-panel.md)

## 🛠️ Support

*   [Troubleshooting](troubleshooting.md)
