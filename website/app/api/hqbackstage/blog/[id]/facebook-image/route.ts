...
export async function POST(
  ...
  try {
    ...
    const fileName = `${post.slug}-fb-${lang}.webp`;
    ...
    if (uploadError) {
      console.error('FB image upload error:', uploadError);
    ...
    const column = lang === 'en' ? 'facebook_image_url_en' : 'facebook_image_url_es';
    ...
    if (updateError) {
      console.error('FB image update error:', updateError);
    ...
    console.error('FB image POST error:', error);
