...
The text field accepts your question. You can type it, or click the **microphone button** to record a voice query (up to 30 seconds). dIKta.me transcribes your question and fills the field automatically.

If you leave the field empty, the default query is used: *"Describe what you see and extract any visible text."*

### Local / Cloud Toggle

Switch between **Local** (your configured Ollama vision model, runs on-device) and **Cloud** (Gemini, Claude, or OpenAI) for each individual capture.
...
*   **Move the mouse** to see a live magnifier showing the exact pixel colour under the cursor, with its hex and RGB values.
...
*   **Tab** to finish and send the palette to the AI for a colour analysis.
*   **Esc** to cancel (if you haven't picked any colours yet) or finish with the current palette.

## Video Recording (Capture Moments)

Start a recording from the Vision Action panel by clicking **Record**, or use your configured video hotkey.

The snipping overlay appears so you can select a region or full screen. Once confirmed, a compact floating bar appears at the top of the screen showing:
...
When you click **Stop**, a post-capture panel appears:

| Button | What it does |
|---|---|
| **Save** | Saves the MP4 file locally. No AI processing. |
| **Describe** | Uploads the clip to Gemini and returns a description of what happened. |
| **Document** | Asks Gemini to write step-by-step instructions for the actions shown. |
...
> [!NOTE]
> Video AI actions require a cloud connection. The **Save** action always works offline.

## Output

By default, Vision responses are **injected at your cursor**, exactly like Dictation. You can change the default per-action in **Settings → Vision**:

| Mode | Behaviour |
|---|---|
| **Inject** (default) | Response is typed into the active window at the cursor. |
...
If you have Ollama installed, you can process images entirely on your own machine. Recommended models:

| Model | Ollama command | VRAM | Best for |
|---|---|---|---|
| `minicpm-v` (default) | `ollama pull minicpm-v` | ~2 GB | General use, OCR, description |
| `moondream` | `ollama pull moondream` | ~1.2 GB | Fast descriptions on low-VRAM hardware |
